...
export function getCheckoutError(state) {
  return state.cart.checkoutState.error;
}

export function isCheckoutPending(state) {
  return state.cart.checkoutState.checkoutPending;
}
...
export default function root(state, action) {
  if (action.type === ADD_TO_CART && state.products.byId[action.products].inventory <= 0)
    return state;
